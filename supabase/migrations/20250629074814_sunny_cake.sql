/*
  ...
  1. New Tables
    - `qr_scans`
      ...
      - `qr_code_id` (uuid, foreign key) - References the QR code that was scanned
      - `user_agent` (text) - Browser/device information
      - `ip_address` (inet) - IP address of scanner (anonymized)
      - `location` (jsonb) - Geographic location data (city, country)
      - `referrer` (text) - Source of the scan
      - `scanned_at` (timestamptz) - When the scan occurred
      - `session_id` (text) - Unique session identifier

  2. Schema Updates
    - Add `tracking_url` column to `qr_codes` table
    - Add `scan_count` column for quick access to total scans
    - Add `last_scanned_at` column for recent activity tracking

  3. Security
    - Enable RLS on `qr_scans` table
    - Add policies for authenticated users to read their own scan data
    - Add policies for public scan logging (insert only)

  4. Indexes
    - Index on qr_code_id for fast scan lookups
...
CREATE INDEX IF NOT EXISTS idx_qr_scans_daily ON qr_scans(qr_code_id, date_trunc('day', scanned_at));
