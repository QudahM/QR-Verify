/*
  # QR Code Analytics System

  1. New Tables
    - `qr_scans` - Logs every scan event with metadata including user agent, IP, location, referrer, and session ID
    
  2. Enhanced Tables
    - `qr_codes` - Added tracking_url, scan_count, and last_scanned_at columns
    
  3. Security
    - Enable RLS on qr_scans table
    - Add policies for users to read their own scan data
    - Allow public scan logging for tracking functionality
    
  4. Performance
    - Add indexes for efficient analytics queries
    - Create triggers for automatic scan count updates
    
  5. Analytics Functions
    - get_daily_scan_analytics: Returns daily scan counts over specified period
    - get_scan_summary: Returns comprehensive scan statistics
*/
...
-- Create policies for qr_scans (with existence checks)
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can read own QR scan data'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'qr_scans' 
    AND policyname = 'Allow public scan logging'
  ) THEN
...
-- Create indexes for performance (removed problematic date_trunc index)
...
-- Trigger to automatically update scan stats
...
BEGIN
  ...
  LEFT JOIN qr_scans qs ON 
    qs.scanned_at::date = ds.scan_date
